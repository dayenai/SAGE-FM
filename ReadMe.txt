LIST-FM: A lightweight and interpretable spatial transcriptomics foundation model
Authors: Xianghao Zhan, Jingyu Xu, Yuanning Zheng
Contact: xzhan96@stanford.edu

This repository is associated with the manuscript entitled “LIST-FM: A lightweight and interpretable spatial transcriptomics foundation model.”
...
•	LIST-FM_Pretraining Evaluation and Get Embeddings_HEST1k.py (evaluate masked gene prediction and export embeddings)
•	LIST-FM_Missing Gene Imputation.py (systematic missingness imputation)
•	LIST-FM_In silico Perturbation.py (in silico perturbation ligand receptor experiments)
•	LIST-FM_perturbation_downstream.py (in silico perturbation ligand downstream genes experiments)
•	LIST-FM_perturbation_downstream.py (in silico perturbation ligand downstream genes experiments)
• LIST-FM_perturbation_baseline.py (in silico perturbation ligand downstream gene genes baseline)

Run Inference on New Data
For users who would like to infer LIST-FM embeddings for a new Visium spatial transcriptomics dataset, please use the Post-training Inference/ folder.
...
After generating subgraphs, run LIST-FM_New Dataset Inference.py to produce LIST-FM embeddings for each subgraph.
...
python "Post-training Inference/LIST-FM_New Dataset Inference.py" \
...
If your dataset name is not GBM, you can either (a) rename your dataset key to GBM, or (b) edit the data_dir_dict inside LIST-FM_New Dataset Inference.py to add your dataset name and path. 
