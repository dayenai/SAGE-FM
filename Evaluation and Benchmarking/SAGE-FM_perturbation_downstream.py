...
def evaluate_perturbation_with_generator(
    ...
    if output_dir_test is None:
        output_dir_test = '/scratch/users/xzhan96/HEST1k_test_perturbation'
    if output_dir_val is None:
        output_dir_val = '/scratch/users/xzhan96/HEST1k_val_perturbation'
    ...
    for perturbation_gene in perturbation_dict.keys():
        ...
        for perturbation_level in perturbation_levels:  # Only run specified perturbation levels
            ...
            # Get all output gene indices for this input gene (all downstream genes in the pathway)
            output_gene_indices = perturbation_dict[perturbation_gene]['output_indices']  # Array of all output gene indices for this input gene
            unmatched_gene_indices = perturbation_dict[perturbation_gene]['unmatched_indices']  # Array of unmatched output gene indices
            output_gene_names = perturbation_dict[perturbation_gene]['output_genes']  # List of output gene names
            
            print(f"Output genes for {perturbation_gene}: {output_gene_names}")
            print(f"Number of downstream genes: {len(output_gene_indices)}")

            # 3. Create the data generators for validation, and testing
            # Use all output gene indices for this input gene to mask them in the central spot
            val_dataset = DataGenerator(data_dir, test_version, 'val', perturbation_gene, perturbation_dict, output_gene_indices, perturbation_level)
            test_dataset = DataGenerator(data_dir, test_version, 'test', perturbation_gene, perturbation_dict, output_gene_indices, perturbation_level)

            val_loader = DataLoader(val_dataset, batch_size=batch_size)
            test_loader = DataLoader(test_dataset, batch_size=batch_size)
            
            # 4. Make the prediction under the perturbation and store the result on the test set
            print(f"Evaluating model: {model_name} on {test_version} with perturbation gene: {perturbation_gene} at level {perturbation_level}")
            log_memory_usage("Before Evaluation")

            model = model.to(device)
            model.eval()
            y_test_matched_gene_expressions = [] # Matched gene expressions for the central spot w.r.t. the input gene (all downstream genes)
            ...
            with torch.no_grad():
                for data in test_loader:
                    ...
                    # Collect predictions for all downstream genes (all output gene indices for this input gene)
                    y_test_matched_gene_expressions.append(predictions.cpu()[::15, output_gene_indices]) # Shape: [batch_size, num_downstream_genes]
                    y_test_unmatched_gene_expressions.append(predictions.cpu()[::15, unmatched_gene_indices]) # Unmatched gene expressions for the central spot w.r.t. the input gene (only central spots)
            
            y_test_matched_gene_expressions = torch.cat(y_test_matched_gene_expressions, dim=0)  # Shape: [num_samples, num_downstream_genes]
            ...
            np.save(f"{output_dir_test}/version_{test_version}_{mask_percentage}_matched_gene_expressions_{perturbation_gene}_level_{perturbation_level}_perturbation_value_{perturbation_value}.npy", y_test_matched_gene_expressions.numpy())
            np.save(f"{output_dir_test}/version_{test_version}_{mask_percentage}_unmatched_gene_expressions_{perturbation_gene}_level_{perturbation_level}_perturbation_value_{perturbation_value}.npy", y_test_unmatched_gene_expressions.numpy())
            ...
            print(f"Evaluating model: {model_name} on {test_version} with perturbation gene: {perturbation_gene} at level {perturbation_level} on validation set")
            ...
            with torch.no_grad():
                for data in val_loader:
                    ...
                    predictions, _ = model(data)
                    # Collect predictions for all downstream genes (all output gene indices for this input gene)
                    y_val_matched_gene_expressions.append(predictions.cpu()[::15, output_gene_indices])  # Shape: [batch_size, num_downstream_genes]
                    y_val_unmatched_gene_expressions.append(predictions.cpu()[::15, unmatched_gene_indices])
            y_val_matched_gene_expressions = torch.cat(y_val_matched_gene_expressions, dim=0)  # Shape: [num_samples, num_downstream_genes]
            ...
            np.save(f"{output_dir_val}/version_{test_version}_{mask_percentage}_matched_gene_expressions_{perturbation_gene}_level_{perturbation_level}_perturbation_value_{perturbation_value}.npy", y_val_matched_gene_expressions.numpy())
            np.save(f"{output_dir_val}/version_{test_version}_{mask_percentage}_unmatched_gene_expressions_{perturbation_gene}_level_{perturbation_level}_perturbation_value_{perturbation_value}.npy", y_val_unmatched_gene_expressions.numpy())
...
def main():
    ...
    parser.add_argument('--output_dir_test', type=str, default=None, help='Output directory for test set results. Default: /scratch/users/xzhan96/HEST1k_test_perturbation')
    parser.add_argument('--output_dir_val', type=str, default=None, help='Output directory for validation set results. Default: /scratch/users/xzhan96/HEST1k_val_perturbation')
