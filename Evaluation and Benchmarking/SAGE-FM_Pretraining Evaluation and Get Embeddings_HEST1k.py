...
class DataGenerator(torch.utils.data.IterableDataset):
    ...
    def __init__(self, data_dir, dataset, split, mask_percentage=0.3, seed=9001):
        ...
        self.dataset = dataset # Datset: HEST1k
    ...
    def __iter__(self):
        for file in self.files:
            subgraphs = torch.load(os.path.join(self.data_dir, file)) # A list of subgraphs: number of spots X number of neighbors X number of gene features
            labels = [file.split('_')[0]] * len(subgraphs)
            for subgraph, label in zip(subgraphs, labels):
                subgraph.y = torch.tensor([int(label)])
...
def evaluate_get_embedding_with_generator(
    ...
    batch_size = int(model_name.split("batch_")[1].split('_')[0])
    ...
    val_dataset = DataGenerator(data_dir, test_dataset, 'val', 0)
    test_dataset = DataGenerator(data_dir, test_dataset, 'test', 0)
    ...
    os.makedirs('./HEST1k_test_subgraph_embeddings', exist_ok=True)
    np.save(f"./HEST1k_test_subgraph_embeddings/{model_name.split('.pt')[0]}_subgraph_embedding.npy", y_test_embeddings.numpy())
    np.save(f"./HEST1k_test_subgraph_embeddings/{model_name.split('.pt')[0]}_subgraph_labels.npy", np.array(y_test_label))
    ...
    test_dataset = DataGenerator(data_dir, test_dataset, 'test', mask_percentage)
    test_loader = DataLoader(test_dataset, batch_size=batch_size)
    ...
    #print('Y Shape: ', y_test_label.shape)
    ...
    os.makedirs('./HEST1k_test_subgraph_embeddings', exist_ok=True)
    np.save(f"./HEST1k_test_subgraph_embeddings/{model_name.split('.pt')[0]}_subgraph_gt.npy", y_test.numpy())
    np.save(f"./HEST1k_test_subgraph_embeddings/{model_name.split('.pt')[0]}_subgraph_prediction.npy", y_test_pred.numpy())
    np.save(f"./HEST1k_test_subgraph_embeddings/{model_name.split('.pt')[0]}_subgraph_mask.npy", y_test_mask.numpy())

    log_memory_usage("After testing")

    del y_test, y_test_pred, y_test_mask, y_test_masked, test_metrics, test_metrics_baseline

    # 5. Get the embeddings on the validation set
    y_val_embeddings = []
    y_val_label = []
    ...
    with torch.no_grad():
        for data in val_loader:
            data = data.to(device)
            _, embeddings = model(data)
            y_val_embeddings.append(embeddings.cpu()[::15]) # Embeddings for the central spot's gene expression features (hidden state) (only central spots)
            y_val_label.append(data.y) # Spot labels (only central spots)
        
    y_val_embeddings = torch.cat(y_val_embeddings, dim=0)
    y_val_label = np.concatenate(y_val_label, axis=0)

    print('Validation Embeddings Shape:', y_val_embeddings.size())
    print('Validation Labels Shape:', y_val_label.shape)

    os.makedirs('./HEST1k_val_subgraph_embeddings', exist_ok=True)
    np.save(f"./HEST1k_val_subgraph_embeddings/{model_name.split('.pt')[0]}_subgraph_embedding.npy", y_val_embeddings.numpy())
    np.save(f"./HEST1k_val_subgraph_embeddings/{model_name.split('.pt')[0]}_subgraph_labels.npy", np.array(y_val_label))

    # Print the RAM usage
    log_memory_usage("Before calculating metrics")

    del val_dataset, val_loader, y_val_embeddings, y_val_label

    # 6. Evaluate the performance on the validation set by masking the central spot expression
    val_dataset = DataGenerator(data_dir, test_dataset, 'val', mask_percentage)
    ...
    with torch.no_grad():
        for data in val_loader:
            data = data.to(device)
            predictions, embeddings = model(data)
            y_val.append(data.original_x.cpu()[::15]) # Original gene expression features (only central spots)
            y_val_pred.append(predictions.cpu()[::15]) # Predicted gene expression features (only central spots)
            y_val_mask.append(data.mask.cpu()[::15]) # Mask for the central spot's gene expression features (only central spots)
            y_val_masked.append(data.x.cpu()[::15]) # Masked gene expression features (only central spots)
    ...
    y_val_mask = torch.cat(y_val_mask, dim=0)
    y_val_masked = torch.cat(y_val_masked, dim=0)

    print('Y_Val Shape: ', y_val.size())
    print('Y_Val Prediction Shape: ', y_val_pred.size())
    #print('Y Shape: ', y_val_label.shape)

    val_metrics = calculate_metrics(y_val_pred, y_val, y_val_mask)
    val_metrics_baseline = calculate_metrics(y_val_masked, y_val, y_val_mask)

    print(f"Validation Set Metrics: MSE: {val_metrics['MSE']:.6f}, RMSE: {val_metrics['RMSE']:.4f}, R2_mean: {val_metrics['R2_mean']:.4f}, R2_median: {val_metrics['R2_median']:.4f}, R2_std: {val_metrics['R2_std']:.4f}")
    print(f"Validation Set Metrics Baseline: MSE: {val_metrics_baseline['MSE']:.6f}, RMSE: {val_metrics_baseline['RMSE']:.4f}, R2_mean: {val_metrics_baseline['R2_mean']:.4f}, R2_median: {val_metrics_baseline['R2_median']:.4f}, R2_std: {val_metrics_baseline['R2_std']:.4f}")

    # 4.4 Save the embeddings and labels
    os.makedirs('./HEST1k_val_subgraph_embeddings', exist_ok=True)
    np.save(f"./HEST1k_val_subgraph_embeddings/{model_name.split('.pt')[0]}_subgraph_gt.npy", y_val.numpy())
    np.save(f"./HEST1k_val_subgraph_embeddings/{model_name.split('.pt')[0]}_subgraph_prediction.npy", y_val_pred.numpy())
    np.save(f"./HEST1k_val_subgraph_embeddings/{model_name.split('.pt')[0]}_subgraph_mask.npy", y_val_mask.numpy())
    
    log_memory_usage("After validation")

    del y_val, y_val_pred, y_val_mask, y_val_masked, val_metrics, val_metrics_baseline
