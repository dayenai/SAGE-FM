...
def generate_deterministic_missingness(data, missingness_percentage=0.3):
    """
    Generate a mask for several columns of gene expression and retain original data as ground truth.

    Parameters:
    - data: The Data object containing gene expression features.
    - missingness_percentage: The percentage of gene expression features to mask.

    Returns:
    - data: The modified Data object with masked features and original data retained.
    """
    num_cells, num_gene_features = data.x.shape

    # Clone the original data and apply the mask
    original_x = data.x.clone()  # Keep a copy of the original data
    masked_x = data.x.clone()
    gene_mask = torch.rand(num_gene_features) < missingness_percentage
    masked_x[:, gene_mask] = 0  # Mask the selected columns of gene expression
    
    # Create a mask tensor with the same shape as data.x
...
class DataGenerator(torch.utils.data.IterableDataset):
    ...
    """
    def __init__(self, data_dir, dataset, split, missingness_percentage=0.3, seed=9001):
        self.data_dir = data_dir # Directory containing the subgraph data
        self.dataset = dataset # Name of the dataset: V1 Small Intersection of small dataset, V2 UCE, V3 large intersection of small dataset, V4 HEST1k
    ...
    def __iter__(self):
        for file in self.files:
            ...
            for subgraph, label in zip(subgraphs, labels):
                ...
                subgraph = generate_deterministic_missingness(subgraph, missingness_percentage=self.missingness_percentage)
                yield subgraph
...
def evaluate_imputation_with_generator(
    model_name, 
    test_dataset,
    ...
    test_dataset = DataGenerator(data_dir, test_dataset, 'test', missingness_percentage=missingness_percentage)
    ...
    # 6. Evaluate the performance on the validation set by masking the gene expression
    val_dataset = DataGenerator(data_dir, test_dataset, 'val', missingness_percentage=missingness_percentage)
...
def main():
    ...
    parser.add_argument('--missingness_percentage', type=float, default=0.3, help='Percentage of missingness for the central spot')
    args = parser.parse_args()

    # Run the evaluation pipeline
    evaluate_imputation_with_generator(args.model_name, args.test_dataset, args.missingness_percentage)
